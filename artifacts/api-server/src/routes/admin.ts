  ...
  try {
    ...
    const enriched = await Promise.all(
      classes.map(async (cls) => {
        const [bookingRows, signinRows] = await Promise.all([
          db
            .select()
            .from(bookingsTable)
            .where(and(eq(bookingsTable.classId, cls.id), eq(bookingsTable.status, "active"))),
          db
        ...
        return {
          ...
          enrollmentCount: bookingRows.length,
...
router.delete("/admin/classes/:classId", async (req, res) => {
  ...
  try {
    await db.delete(bookingsTable).where(eq(bookingsTable.classId, parsed.data.classId));
    await db
...
router.get("/admin/overview", async (req, res) => {
  if (!(await requireAdmin(req, res))) return;
  try {
    const [membersRes, coachesRes, adminsRes, classesRes, bookingsRes] = await Promise.all([
      ...
      db.select({ count: count() }).from(bookingsTable).where(eq(bookingsTable.status, "active")),
    ]);
    res.json({
      ...
      totalActiveBookings: bookingsRes[0]?.count ?? 0,
    });
  } catch (err) {

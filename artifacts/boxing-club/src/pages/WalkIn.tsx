...
import { CheckCircle2, Clock, MapPin, User, ChevronRight, RotateCcw, Search } from "lucide-react";
...
export default function WalkIn() {
  ...
  const [members, setMembers] = useState<{ firstName: string; lastName: string; email: string }[]>([]);
  const [memberQuery, setMemberQuery] = useState("");

  useEffect(() => {
    fetch("/api/walkins/members")
      .then((r) => r.json())
              ...
              <CardContent>
                {members.length > 0 && (
                  <div className="mb-4 pb-4 border-b border-border/50">
                    <Label htmlFor="memberSearch" className="text-xs uppercase tracking-wider text-muted-foreground mb-2 block">
                      Already a member? Find your name
                    </Label>
                    ...
                    {memberQuery.trim().length >= 1 && (
                      <div className="mt-2 max-h-44 overflow-y-auto border border-border/50 rounded-md divide-y divide-border/30">
                        {members
                          .filter((m) => {
                        ...
                        {members.filter((m) => {
                          ...
                          <p className="px-3 py-2 text-xs text-muted-foreground italic">No match — type your info below.</p>
                        )}
                      </div>
                    )}
                  </div>
